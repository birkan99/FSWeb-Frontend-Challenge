import React from "react";
import { SKILLS_CONTENT } from "../config"; 

const Skills = () => {
  return (
    <section className="bg-white py-20 px-4 md:px-8 lg:px-12">
      <div className="max-w-6xl mx-auto">
        
        {/* Ana Başlık */}
        <h2 className="text-5xl font-extrabold text-gray-900 mb-8 text-left">
          {SKILLS_CONTENT.title}
        </h2>

        {/* Yetenek Kartları (3 Sütunlu Izgara) */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-16 gap-y-12">
          
          {SKILLS_CONTENT.skills.map((skill, index) => (
            // Her bir yetenek kartı
            <div key={index} className="flex flex-col text-left">
              
              {/* Yetenek Adı -*/}
              <h3 className="text-2xl font-semibold text-[#4B30A4] mb-2">
                {skill.name}
              </h3>
              
              {/* Açıklama Paragrafı - */}
              <p className="text-sm text-gray-600 leading-relaxed">
                {skill.description}
              </p>
            </div>
          ))}
        </div>

        {/* Ayırıcı Çizgi */}
        <div className="mt-20 h-0.5 w-full bg-[#E0E0FF]"></div>

      </div>
    </section>
  );
};

export default Skills;